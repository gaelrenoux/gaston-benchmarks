...
object AssignmentLoop {
  ...
  def newSchedule: Schedule = ScheduleMaker.fromOldSchedule(oldSchedule, newProblem)

  given Printable[Schedule] = SchedulePrinter(newProblem)

  val newImprover = new AssignmentImprover(newProblem)

  def improveNewSchedule(): Unit = while (true) {
    newImprover.improve(newSchedule)
  }

  @main
  def main(args: String*): Unit = {
    println("!!! I'm improving a new schedule !!!")
    improveNewSchedule()
  }
}
